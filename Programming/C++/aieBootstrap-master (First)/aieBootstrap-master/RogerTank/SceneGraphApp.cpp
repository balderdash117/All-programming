...
bool SceneGraphApp::startup() {
	...
	m_bullets = new aie::Texture("./sceneGraph/textures/bullet1.png");
	

	float windowWidth = (float) getWindowWidth() / 2;
	float windowHieght = (float) getWindowHeight() / 2;
...
void SceneGraphApp::update(float deltaTime) {
	...
	float l_TimeInterval = 0.30f;

	if (input->isKeyDown(aie::INPUT_KEY_SPACE) && (currentTime > l_TimeInterval)) 
	{
		currentTime = 0;
...
void SceneGraphApp::draw() {
	...
	m_2dRenderer->setRenderColour(0xffffff);
	for (auto iter = m_bullet.begin(); iter != m_bullet.end(); ++iter) {
