...
void AsteroidsApp::update(float deltaTime) {
	...
	if (input->isKeyDown(aie::INPUT_KEY_LEFT) || input->isKeyDown(aie::INPUT_KEY_A))
	{
		...
		mPlayer.mX += xDir * mPlayer.mSpeed * deltaTime;
		mPlayer.mY += yDir * mPlayer.mSpeed * deltaTime;
...
void AsteroidsApp::draw() {
	...
	if (mBall.mX != -1)
	{
		m_2dRenderer->setRenderColour(DK_CYAN);
		m_2dRenderer->drawCircle(mBall.mX, mBall.mY, mBall.mRadius, 0.0f);
	}
	...
	for (unsigned int i = 0 ; i < mAst.size(); ++i)
	{
		...
		m_2dRenderer->drawSprite(m_asteroid, mAst[i].mX, mAst[i].mY, mAst[i].mRadius * 5, mAst[i].mRadius * 5, 0.0f);
	}

	m_2dRenderer->setRenderColour(WHITE);
	//m_2dRenderer->drawCircle(mPlayer.mX, mPlayer.mY, mPlayer.mRadius, 0.0f);

	m_2dRenderer->drawSprite(m_myImage, mPlayer.mX, mPlayer.mY, 65.0f, 65.0f, m_rotation);
	
	
	m_2dRenderer->setUVRect(0, 0, 1, 1);
	m_2dRenderer->drawSprite(m_backing, getWindowWidth() / 2, getWindowHeight() / 2, getWindowWidth(), getWindowHeight(), 0.f, 5.f);
...
void AsteroidsApp::testCreateAsteroid(float a_deltaTime)
{
	...
	{
		mPlayer.mRadius = std::max(mBall.mRadius, mPlayer.mRadius - 1.f);
