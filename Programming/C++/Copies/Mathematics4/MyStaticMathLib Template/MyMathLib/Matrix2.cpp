...
{
	return Matrix2(1.0f, 0.0f, a_x, a_y);
}
Matrix2 Matrix2::createTranslation(const Vector2 &a_vec)
{
	return Matrix2(1.0f, 0.0f, a_vec.x, a_vec.y);
}


Matrix2 Matrix2::createRotation(float a_rot)
{
	//Look up google over the weekend
	return Matrix2(cosf(a_rot), sinf(a_rot), -sinf(a_rot), cosf(a_rot));
}

Matrix2 Matrix2::createScale(float a_xScale, float a_yScale)
{
	return Matrix2(a_xScale, 0.0f, 0.0f, a_yScale);
}

Matrix2  Matrix2::operator * (const Matrix2 &a_rhs) const
{
	return Matrix2(m1*a_rhs.m1 + m3*a_rhs.m2, m2*a_rhs.m1 + m4*a_rhs.m2, m1*a_rhs.m3 + m3*a_rhs.m4, m2*a_rhs.m3 + m4*a_rhs.m4);
}
...
{
	return Vector2(m1*a_vec.x + m3*a_vec.y, m2*a_vec.x + m4*a_vec.y);
}

//Matrix2::operator float* ()
//{
//	//return pointer to start of matrix
//	return m;
//}
